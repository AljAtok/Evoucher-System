			...
			<div class="tab-pane fade show active" id="dashboard-coupons" role="tabpanel" aria-labelledby="nav-product-coupon-tab">
				...
				<nav>
                    <div class="nav nav-tabs" id="nav-tab" role="tablist">
						...
						<a class="nav-link" id="nav-dash-inventory-tab" data-toggle="tab" href="#nav-dash-inventory" role="tab" aria-controls="nav-dash-inventory" aria-selected="false">
							<span class="fas fa-fw fa-cubes"></span>  INVENTORY
                        </a>
                    </div>
                </nav>
				<div class="tab-content mt-2">
										...
										<div class="row">
											...
											<div class="col-xl-8">
												<div class="chart-container">
													...
													<canvas id="volBasedOnPaymentType" height="255"></canvas>
												</div>
											</div>
										</div>
									</div>
										...
										<div class="row">
											...
											<div class="col-xl-8">
												<div class="chart-container">
													...
													<canvas id="amountBasedOnPaymentType"></canvas>
												</div>
											</div>
										</div>
									</div>
					...
					<div class="tab-pane fade show" id="nav-dash-credit" role="tabpanel" aria-labelledby="nav-dash-credit-tab">
						<div class="tab-pane fade show active" id="credit-coupons" role="tabpanel" aria-labelledby="nav-product-coupon-tab">
							<nav>
								<div class="nav nav-tabs" id="nav-tab" role="tablist">
								...
								<div class="tab-pane fade show active" id="nav-receivables" role="tabpanel" aria-labelledby="nav-receivables-tab">
									...
									<div class="table-responsive">
										<table class="table table-striped table-condensed" width="100%" id="tbl_dashboard_receivables">
											...
											<tbody>
												<!-- <?php foreach($unpaid_coupon_trans as $row):
												...
												<tr>
													...
													<td><?= $row->active_coupon_qty ?></td>
													<td><?= $row->coupon_use ?></td>
													<td><?= decimal_format($row->coupon_value) ?></td>
													<td><?= decimal_format($row->total_coupon_value) ?></td>
													<td><?= $row->payment_terms.' DAYS'?></td>
													<td><?= add_date($row->coupon_transaction_header_added, 0, 0, $row->payment_terms) ?></td>
													<td><?= $row->user_lname . ', ' . $row->user_fname ?></td>
													<td><?= date_format(date_create($row->coupon_transaction_header_added),"M d, Y h:i A");?></td>
													<td class="d-flex d-inline justify-content-center align-items-center"><?= (in_array($row->coupon_cat_id, paid_category())) ? $payment_badge : '-' ?></td>
								...
								<div class="tab-pane fade" id="nav-cleared" role="tabpanel" aria-labelledby="nav-cleared-tab">
									...
									<div class="table-responsive">
										<table class="table table-striped table-condensed data-table">
											...
											<tbody>
												<?php foreach($paid_coupon_trans as $row):
														if($row->payment_status == 1){
															$payment_badge = '<span class="badge badge-success">Paid</span>';
														}elseif($row->payment_status == 0){
															$payment_badge = '<span class="badge badge-warning">Unpaid</span>';
					...
					<div class="tab-pane fade show" id="nav-dash-non-credit" role="tabpanel" aria-labelledby="nav-dash-non-credit-tab">
						...
						<div class="table-responsive">
							<table class="table table-striped table-condensed data-table">
								...
								<tbody>
									<?php foreach($non_credit_coupon_trans as $row):
											if($row->payment_status == 1){
												$payment_badge = '<span class="badge badge-success">Paid</span>';
											}elseif($row->payment_status == 0){
												$payment_badge = '<span class="badge badge-warning">Unpaid</span>';
