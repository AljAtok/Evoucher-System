...
async function drawWinner() {
	try {
		...
		const itemHeight = 50;
		const revealDelayPerChar = 500; // Slower reveal between boxes
		const animationDuration = 9000;
		const spinDuration = 10000; // Longer spin duration for more drama
		...
		// Populate each spinner so the winner char is at the VISIBLE CENTER after spin
		charSpinners.forEach((spinner, index) => {
            let charsToSpin = [];
            let charSet = index < 6 ? numericChars : alphanumericChars; // 7-9 are alphanumeric
            const spinCount = 100; // long enough for a real spin
            for (let i = 0; i < spinCount; i++) {
                charsToSpin.push(charSet[Math.floor(Math.random() * charSet.length)]);
            }
            // Winner char is always the last
            charsToSpin.push(winnerRefChars[index]);
            spinner.innerHTML = charsToSpin
                .map((char) => `<div class=\"char-item\" style=\"height:50px;line-height:50px;\">${char}</div>`)
                .join("");
        ...
        await new Promise((resolve) => {
            ...
            for (let i = 0; i < raffleCharBoxes.length; i++) {
                ...
                const targetIndex = 100; // winner char is at index 100
                setTimeout(() => {
                    spinner.style.transition = `transform ${spinDuration}ms cubic-bezier(0.25, 0.1, 0.25, 1)`;
                    spinner.style.transform = `translateY(-${targetIndex * itemHeight}px)`;
                    setTimeout(() => {
                        spinner.style.transition = "none";
                        // Instantly replace spinner with winner char, then fade in winner char
                        spinner.innerHTML = `<div class='char-item' style='height:50px;line-height:50px;opacity:0;transition:opacity 0.3s;'>${winnerRefChars[boxIndex]}</div>`;
                        spinner.style.transform = `translateY(0px)`;
                        setTimeout(() => {
                            const charDiv = spinner.querySelector('.char-item');
                            if (charDiv) charDiv.style.opacity = '1';
                            revealedCount++;
                            if (revealedCount === 9) {
                                raffleCharBoxes.forEach(box => box.classList.add("winner-highlight"));
                                resolve();
                            }
                        }, 50);
                    }, spinDuration + 50);
