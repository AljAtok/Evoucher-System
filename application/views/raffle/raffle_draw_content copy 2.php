...
async function drawWinner() {
    try {
        ...
        if (!apiResponse.ok) {
            showMessage(result.messages.error || "Failed to draw winner", "error");
            drawRaffleBtn.disabled = false;
            return;
        }
        ...
        // --- Per-box interval animation ---
        let intervals = [];
        let revealDelay = 120; // ms between each box reveal
        let spinTime = 1800; // ms total spin per box
        let now = Date.now();

        // Start all boxes spinning random chars
        for (let i = 0; i < 9; i++) {
            let charSet = i < 6 ? numericChars : alphaChars;
            intervals[i] = setInterval(() => {
                charSpans[i].textContent = charSet[Math.floor(Math.random() * charSet.length)];
            }, 40);
        }

        // Stop each box one by one, revealing the winning char
        for (let i = 0; i < 9; i++) {
            setTimeout(() => {
                clearInterval(intervals[i]);
                charSpans[i].textContent = winnerRefChars[i];
                raffleCharBoxes[i].classList.add("winner-highlight");
            }, spinTime + i * revealDelay);
        }

        // Wait for all boxes to finish
        await new Promise((resolve) => setTimeout(resolve, spinTime + 9 * revealDelay + 300));
...
function renderParticipants() {
	...
	if (participants.length === 0) {
		...
		participants.forEach((p) => {
			...
			// Use string concatenation for HTML, not template literals
			card.innerHTML =
				'<span>' + p.name + ' (' + (p.ref_no ? p.ref_no : 'N/A') + ')</span>';
...
function createConfetti() {
	...
	for (let i = 0; i < confettiCount; i++) {
		...
		confetti.style.setProperty(
			"--x",
			(String)((Math.random() * window.innerWidth - (window.innerWidth / 2))) + "px"
		);
		confetti.style.setProperty("--y", (String)((Math.random() * -50)) + "px"); // Start above the screen
		confetti.style.setProperty(
			"--x-end",
			(String)((Math.random() * window.innerWidth - (window.innerWidth / 2))) + "px"
		);
		confetti.style.setProperty("--y-end", (String)((window.innerHeight + 100)) + "px"); // Fall below the screen
