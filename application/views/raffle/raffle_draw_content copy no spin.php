...
async function drawWinner() {
	try {
		...
		// --- Slot flicker effect ---
		let intervals = [];
		let revealDelay = 220; // ms between each box reveal
		let spinTime = 3800; // ms total spin per box

		// Start all boxes spinning random chars
		for (let i = 0; i < 9; i++) {
			// let charSet = i < 6 ? numericChars : alphaChars;
			let charSet = alphaNumericChars;
		...
		await new Promise((resolve) =>
			setTimeout(resolve, spinTime + 9 * revealDelay + 400)
		);
