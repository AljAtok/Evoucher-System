...
async function renderTabs() {
	...
	renderTable(
		...
		[
			...
			{ label: "Timestamp", key: "entry_created_at" },
		],
		(row) =>
	...
	renderTable(
		...
		(row) =>
			`<button class='btn btn-secondary btn-xs mr-1' onclick='confirmBackToNotValidated("${
				row.id
			}", "${
				row.name
			}")'>Back to Not Validated</button><button class='btn btn-danger btn-xs mr-1' onclick='confirmRejectWinner("${
				row.id
			}", "${
				row.name
			}")'>Reject</button><button class='btn btn-primary btn-xs' onclick='showParticipantInfo(${JSON.stringify(
...
window.confirmValidateWinner = function (id, name) {
	showModal(
		`Winner <b>${name}</b> is already validated?`,
...
window.showParticipantInfo = function (row) {
	let info = Object.entries(row)
		.map(([k, v]) => `<div><b>${k}:</b> ${v}</div>`)
		.join("");
	showModal(
		`<div class='mb-2 font-semibold text-lg'>Participant Info</div>${info}`
	);
};
// --- Action Handlers (placeholders) ---
window.validateWinner = function (id) {
	closeModal();
	showMessage(`Validate winner: ${id}`);
};
window.rejectWinner = function (id) {
	closeModal();
	showMessage(`Reject winner: ${id}`);
};
...
if (tabNotValidated && tabValidated && tabParticipants) {
